...
function createClientValidation(req, res, next) {
	const schema = Joi.object({
		bussinessName: Joi.string().required().min(3),
		...
		openFacility: Joi.string().required(),
	});
	validateRequest(req, res, next, schema);
}
function updateClientValidation(req, res, next) {
	const schema = Joi.object({
		bussinessName: Joi.string().required().min(3),
		contactName: Joi.string().required(),
		mobile: Joi.string()
